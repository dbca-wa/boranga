<template id="ocr_external_referrals_dashboard">
    <div>
        <CollapsibleFilters
            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="ocr_ref_scientific_name_lookup"
                            >Scientific Name:</label
                        >
                        <select
                            id="ocr_ref_scientific_name_lookup"
                            ref="ocr_ref_scientific_name_lookup"
                            name="ocr_ref_scientific_name_lookup"
                            class="form-control"
                        />
                    </div>
                </div>
...
export default {
    name: 'OCRExternalReferralsTable',
    components: {
        datatable,
        CollapsibleFilters,
    },
    ...
    data() {
        return {
            datatable_id: 'ocr_external_ref-datatable-' + uuid(),
            ...
            return {
                ...
                render: function (data, type, full) {
                    ...
                    links += full.can_be_processed
                        ? `<a href='/external/occurrence-report/${full.occurrence_report_id}/referral/${full.id}'>Process</a><br/>`
                        : `<a href='/external/occurrence-report/${full.occurrence_report_id}/referral/${full.id}'>View</a><br/>`;
                    return links;
                },
            };
        },
    ...
    watch: {
        filterCSRefFloraScientificName: function () {
            let vm = this;
            vm.$refs.flora_ocr_ref_datatable.vmDataTable.ajax.reload(
                helpers.enablePopovers,
                true
    ...
    mounted: function () {
        ...
        this.$nextTick(() => {
            ...
            if (
                ...
                $('#ocr_ref_scientific_name_lookup').append(newOption);
            }
            if (
    ...
    methods: {
        initialiseScientificNameLookup: function () {
            let vm = this;
            $(vm.$refs.ocr_ref_scientific_name_lookup)
                .select2({
                ...
                .on('select2:open', function () {
                    const searchField = $(
                        '[aria-controls="select2-ocr_ref_scientific_name_lookup-results"]'
                    );
        ...
        addEventListeners: function () {
            let vm = this;
            vm.$refs.flora_ocr_ref_datatable.vmDataTable.on(
                'childRow.dt',
                function () {
                    helpers.enablePopovers();
                }
