<template lang="html">
    <div id="community_documents">
        ...
        <ViewDocument
            ...
            :is_internal="is_internal"
        ></ViewDocument>
        <div v-if="communityDocumentHistoryId">
            <CommunityDocumentHistory
                ref="community_document_history"
                :key="communityDocumentHistoryId"
                :document-id="communityDocumentHistoryId"
                :community-id="species_community.id"
            />
        </div>
    </div>
</template>
...
export default {
            ...
            documents_options: {
                ...
                ajax: {
                    url: helpers.add_endpoint_json(
                        ...
                        vm.species_community.id + '/documents'
                    ),
                    dataSrc: '',
                },
                order: [],
                ...
                buttons: [
                    {
                        extend: 'excel',
                        title: 'Boranga Community Documents Excel Export',
                    ...
                    {
                        extend: 'csv',
                        title: 'Boranga Community Documents CSV Export',
    ...
    methods: {
        ...
        editDocument: function (id) {
            ...
            fetch(
                helpers.add_endpoint_json(api_endpoints.community_documents, id)
            ).then(
                async (response) => {
            ...
            //this.$refs.document_detail.fetchCommunityDocument(id);
            this.$refs.document_detail.isModalOpen = true;
        },
        historyDocument: function (id) {
            this.communityDocumentHistoryId = parseInt(id);
            this.uuid++;
            this.$nextTick(() => {
                this.$refs.community_document_history.isModalOpen = true;
            });
        },
                ...
                if (result.isConfirmed) {
                    fetch(
                        helpers.add_endpoint_json(
                            api_endpoints.community_documents,
                            id + '/discard'
                        ),
                        {
                            method: 'PATCH',
